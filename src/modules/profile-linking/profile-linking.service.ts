...
async function assertOwnership(item: IProfileLinking, actor: Actor): Promise<void> {
...
class ProfileLinkingService {
  ...
  async list(query: ListQueryDto, actor: Actor): Promise<PaginatedResult<IProfileLinking>> {
    return profileLinkingRepository.paginate(query, await this.scope(actor));
  }
  ...
  async updateStatus(id: string, dto: UpdateStatusDto, actor: Actor): Promise<IProfileLinking> {
    if (!canManagePlatformWorkflow(actor as ScopeActor)) {
      throw ApiError.forbidden('Only Super Admin or Sub Admin can change profile linking status');
    }
